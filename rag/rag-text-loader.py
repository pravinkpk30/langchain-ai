...
from langchain.document_loaders import TextLoader  # For loading text files
from langchain.text_splitter import TokenTextSplitter  # For splitting text
...
# This reads the content from 'speech.txt' in the same directory
...
# Using 'all-MiniLM-L6-v2' model which is efficient and effective for general text
...
st.title("LangChain Text Based RAG")

# Create a text input field for user's question
topic = st.text_input("Enter your question about the text document")

# When user enters a question
if topic:
    # Show a spinner while processing
    with st.spinner("Analyzing the document for answers..."):
        try:
            # Get the answer from the QA chain
            result = qa({"query": topic})
